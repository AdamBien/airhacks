...
This repository started from the bce.design brownfield template; the `bookmarks` sample BC has been removed. The application is the Amsterdam Bike Rental frontend, driven by `/sbce`: the `rental` BC's capability spec lives in `app/src/rental/package-info.md` and is converged by `/sbce apply rental`. Coupling points per BC: imports and route registration in `app/src/app.js`, reducer registration in `app/src/store.js`, the page title and nav in `app/src/index.html`, and the Playwright specs in `tests/tests/bce.design.spec.js` and `codecoverage/tests/bce.design.spec.js`.
