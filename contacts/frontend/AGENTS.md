...
The `contacts` BC (`app/src/contacts/`) owns the contacts UI; its spec is the co-located `package-info.md` (SBCE). The app expects the contacts service on `http://localhost:8080` (configured in `app/src/app.config.js`). Coupling points when adding or replacing BCs: imports and route registration in `app/src/app.js`, reducer registration in `app/src/store.js`, the page title in `app/src/index.html`, and the Playwright specs in `tests/tests/` and `codecoverage/tests/`.
