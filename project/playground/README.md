# Airzon Store — Multi-Seller Marketplace
...
|---|---|---|
| **Customer** | Registered buyer | Search, cart across stores, single checkout, track per-seller shipment, submit verified reviews |
...
A customer pays once for their entire cart, regardless of how many sellers it spans. Internally, the platform splits the order into one **sub-order** per seller. Each seller sees and fulfils only their own sub-orders. The customer retrieves a unified order with per-seller shipment status and tracking.
...
- Search products by keyword, price range, category, and sort order
- Retrieve product detail with full variant matrix (options, stock status, image gallery)
...
- Retrieve orders with per-seller shipment tracking
- Cancel orders before any seller ships (full refund)
- Submit verified-purchase reviews with star ratings
- AI-powered product recommendations and natural-language search *(stretch goal)*
...
- Retrieve own sub-orders and mark them as shipped with a tracking number
- Retrieve commission breakdown per sub-order and weekly payout history
...
- Retrieve marketplace-wide metrics: GMV, commission earned, top stores, new customers
...
| 3 | Stores | Store profile management, store API, admin commission & suspend | US11–US14 |
...
| 8 | Metrics | Seller performance metrics API, admin marketplace metrics API | US35–US36 |
| ★ | AI & Intelligence | Search API, recommendations API, description generator, pricing suggest, stock forecast, health report, review summary | US37–US43 |
